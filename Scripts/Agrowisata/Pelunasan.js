...
var menuId = '188abe67-f36e-4b4a-ac79-201e97cf3e14';
    ...
    $("#grdDetail").kendoValidator({
        rules: { // custom rules
            chopvalidation: function (input) {
                if (input.attr("name") == "PelunasanDisetor" && input.val() != "") {
                    var grid = $('#grdDetail').data('kendoGrid');
                    var dataItem = typeof (grid.dataSource.get(pelunasanId)) == "undefined" ? grid.dataSource.getByUid(pelunasanId) : grid.dataSource.get(pelunasanId);
                    dataItem.SisaPelunasan = dataItem.JmlYangHarusDibayar - dataItem.PelunasanDisetor;
                } else {
                    return true;
                };
            }
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        data: {
            strClassView: "Ptpn8.Agrowisata.Models.View_Pelunasan",
            scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[Agrowisata_Pelunasan] " +
                " SELECT top 1 newid() as PelunasanId, A.KodeBooking, A.MetodePembayaran, D.SisaPelunasan as JmlYangHarusDibayar,0 as PelunasanDisetor,0 as SisaPelunasan," +
                " '' as Keterangan, A.ReservasiId, A.Customer_AgrowisataId, C.Nama as NamaCustomer,GETDATE () as TanggalPelunasan " +
                ...
                " order by D.TanggalUangMuka desc ",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function InsertGridFromTanpaUangMuka() {
    ...
    return $.ajax({
        ...
        data: {
            strClassView: "Ptpn8.Agrowisata.Models.View_Pelunasan",
            scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[Agrowisata_Pelunasan] " +
                " SELECT newid() as PelunasanId, A.KodeBooking, A.MetodePembayaran, sum(B.Harga) as JmlYangHarusDibayar,0 as PelunasanDisetor,0 as SisaPelunasan," +
                " '' as Keterangan, A.ReservasiId, A.Customer_AgrowisataId, C.Nama as NamaCustomer,GETDATE () as TanggalPelunasan " +
                " from [SPDK_KanpusEF].[dbo].[Reservasi] as A join [SPDK_KanpusEF].[dbo].[DTL_Reservasi] as B on A.ReservasiId=B.ReservasiId" +
                " join [ReferensiEF].[dbo].[Customer_Agrowisata] as C on A.Customer_AgrowisataId=C.Customer_AgrowisataId" +
                " where A.KodeBooking = '" + kodeBooking + "'" +
...
function sendData() {
    ...
    for (var i = 0; i < currentData.length; i++) {
        if (currentData[i].isNew()) {
            ...
            spupdated.push("EXEC Agrowisata_UbahStatusReservasiPelunasan '" + kodeBooking + "'");
        }
    }
    ...
    for (var i = 0; i < grid.dataSource._destroyed.length; i++) {
        ...
        spdeleted.push("EXEC Agrowisata_UbahStatusReservasiPelunasanBatal '" + kodeBooking + "'");
    }
