var mPP_RasioPabrikCTCId, unitId, err;
var detailBaru = false;
var userName;
var menuId = '1E946EC1-B8F2-44EA-A9A4-414038F00E28';
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.id = model.uid;
        e.container.find("input[name=MPP_RasioPabrikCTCId]").val(model.id).change();
    }
    mPP_RasioPabrikCTCId = model.id;
    unitId = model.UnitId;
...
function sendData() {
    ...
    for (var i = 0; i < currentData.length; i++) {
        if (currentData[i].isNew()) {
            ...
            spupdated.push("EXEC [dbo].[MPP_HitungHasilPabrikCTC] '" + unitId + "'");
        }
    }
