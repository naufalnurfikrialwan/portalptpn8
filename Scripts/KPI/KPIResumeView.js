...
var buyer;
var hdr_KPIResumesId,dtl_KPIResumesId, bagianId;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var rowNumber = 0;
var menuId = '1C373674-1097-40BF-9C2B-A35FEE2C8402';
            ...
            $scope.validate = function (event) {
                ...
                if ($scope.validator.validate()) {
                    simpanHeader().done(function (data) {
                        if (data) {
                            hdr_PJBId = $('#HDR_KPIResumesId').val();
...
function ModelToView(_NomorInputView) {
    $('#HDR_KPIResumesId').val(_NomorInputView.HDR_KPIResumesId);
    $('#BagianId').val(_NomorInputView.BagianId);
    $('#TahunBuku').val(_NomorInputView.TahunBuku);
    $('#Bulan').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.Tanggal_PJB)));
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        hdr_KPIResumesId = _NomorInputView.HDR_KPIResumesId;
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_KPIResumesId: $('#HDR_KPIResumesId').val() }
    });
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_KPIResumesId = model.uid;
        model.HDR_KPIResumesId = hdr_KPIResumesId;
    }
    dtl_KPIResumesId = model.DTL_KPIResumesId;
    bagianId = $('#BagianId').val();
    gridStatus = 'sudah-diapa-apain';
}
...
function ddlBagianOnChange(e) {
    var bagian = $('#BagianId').val();
    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(dtl_KPIResumesId)) == "undefined" ? grid.dataSource.getByUid(dtl_KPIResumesId) : grid.dataSource.get(dtl_KPIResumesId);
