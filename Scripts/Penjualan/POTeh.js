    ...
    var rowNumber = 0;
    var menuId = 'AE6C18F6-31EE-4693-81A1-A54BBC09EDA1';
    ...
    function resetRowNumber(e) {
        ...
        var status = $("#No_PO").data("kendoAutoComplete").value();
        for (var i = 0; i < datasource.length; i++) {
            var model = datasource[i];
            if (status.toLowerCase().indexOf("kpb") == -1) { // jumlah harga dihitung spt biasa
            ...
            else // dibalik
            {
                if (model.MataUang == "IDR") {
                    var prc = model.PriceIDR;
                    model.PriceIDR = hitungHargaSatuan(model.DTL_POId);
                    model.Price = 0;
                    if(prc!=model.PriceIDR) model.dirty = true;
                }
                else {
                    var prc = model.PriceIDR;
                    model.PriceIDR = 0;
                    model.Price = hitungHargaSatuan(model.DTL_POId);
                    if (prc != model.PriceIDR) model.dirty = true;
                }
            }
...
$(window).on("unload", function () {
    ...
    function grdOnEdit(e) {
		...
		else // dibalik
		{
		    if (model.MataUang == "IDR")
		    {
		        model.PriceIDR = hitungHargaSatuan(dtl_POId);
		        model.Price = 0;
		    }
		    else
		    {
		        model.PriceIDR = 0;
		        model.Price = hitungHargaSatuan(dtl_POId);
		    }
		}
    }
    ...
    function NoPOOnSelect(e)
    {
        var grd = $("#grdDetail").data("kendoGrid");
        var status = $("#No_PO").data("kendoAutoComplete").value();
        if(status.toLowerCase().indexOf("kpb")>-1 && grd.columns[20].field=='JumlahHarga')
        {
            grd.reorderColumn(18,grd.columns[20])
        }
        else if (status.toLowerCase().indexOf("kpb")==-1 && grd.columns[20].field == 'PriceIDR')
        {
            grd.reorderColumn(20, grd.columns[18])
        }
    }

    function NoPOOnChange(e)
    {
        var grd = $("#grdDetail").data("kendoGrid");
        var status = $("#No_PO").data("kendoAutoComplete").value();
        if (status.toLowerCase().indexOf("kpb") > -1 && grd.columns[20].field == 'JumlahHarga') {
