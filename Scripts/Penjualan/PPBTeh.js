...
var rowNumber = 0;
var menuId = 'CBC05359-7391-4728-A2CF-22E4FECAED10';
var kebunId;
var unilever = '8D03AF76-EE16-40F1-A99F-9123DB467C90';
    ...
    $("#grdDetail").kendoValidator({
        rules: { // custom rules
            chopvalidation: function (input) {
                if (input.attr("name") == "QtySacks" && input.val() != "") {
                ...
                else if (input.attr("name") == "Price" && input.val() != "") {
                    var grid = $('#grdDetail').data('kendoGrid');
                    var dataItem = typeof (grid.dataSource.get(dtl_PPBId)) == "undefined" ? grid.dataSource.getByUid(dtl_PPBId) : grid.dataSource.get(dtl_PPBId);
                    var hargaPerKg_Harga = dataItem.Price;
                    var gross = dataItem.Gross;
                    var tare = dataItem.Tare;
                    var netto = gross-tare;

                    var hargaJumlah = hargaPerKg_Harga * netto;
                    dataItem.JumlahHarga = hargaJumlah;
                    $('#JumlahHarga').val(hargaJumlah);
...
function InsertGridFrom() {
    ...
    var lstFaktur = $('#ListFakturId').data('kendoMultiSelect').value();
    var strFaktur = "";
    for (var i = 0; i < lstFaktur.length; i++) {
        strFaktur = strFaktur + "'" + lstFaktur[i] + "'";
        if (i < lstFaktur.length - 1) strFaktur = strFaktur + ",";
    }

    var lstPO = $('#ListPOId').data('kendoMultiSelect').value();
    var strPO = "";
    for (var i = 0; i < lstPO.length; i++) {
        strPO = strPO + "'" + lstPO[i] + "'";
        if (i < lstPO.length - 1) strPO = strPO + ",";
    }

    if (kebunId != '00000000-0000-0000-0000-000000000000') {
        if (strFaktur != "" && strPO == "") {
            return $.ajax({
                url: url,
                //contentType: 'application/json; charset=utf-8',
                data: {
                    ...
                    scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[DTL_PPB] " +
                        ...
                        " where HDR_FakturId IN (" + strFaktur + ") and B.KebunId='" + $('#KebunId').val() + "' and A.SudahPPB=0 ",
                    _menuId: menuId
                },
                type: 'POST',
                datatype: 'json'
            ...
            return $.ajax({
                ...
                data: {
                    ...
                    scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[DTL_PPB] " +
                        " SELECT newid() as DTL_PPBId, cast('" + $('#HDR_PPBId').val() + "' as uniqueidentifier) as HDR_PPBId," +
                        " '00000000-0000-0000-0000-000000000000', A.SubBudidayaId, A.MerkId, A.Chop, A.QtySacks," +
                        " A.SatuanId, A.GradeId, A.Gross, A.Tare, Price = case when A.MataUang='IDR' then A.PriceIDR else A.Kurs*A.Price/100 end, A.KK, A.KA, A.ALB, A.TahunProduksi,0,A.DTL_POId" +
                        " from [SPDK_KanpusEF].[dbo].[DTL_PO] as A" +
                        " inner join [ReferensiEF].[dbo].[KebunBudidaya] as B on A.MerkId=B.KebunBudidayaId" +
                        " where HDR_POId IN (" + strPO + ") and B.KebunId='" + $('#KebunId').val() + "' and A.SudahFaktur=0",
                    _menuId: menuId
                },
                type: 'POST',
                datatype: 'json'
            ...
            return $.ajax({
                ...
                data: {
                    ...
                    scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[DTL_PPB] " +
                        ...
                        " from [SPDK_KanpusEF].[dbo].[DTL_Faktur] as A" +
                        " inner join [ReferensiEF].[dbo].[KebunBudidaya] as B on A.MerkId=B.KebunBudidayaId" +
                        " where HDR_FakturId IN (" + strFaktur + ") and A.SudahPPB=0",
                    _menuId: menuId
                },
                type: 'POST',
                datatype: 'json'
            ...
            return $.ajax({
                ...
                data: {
                    ...
                    scriptSQL: "INSERT INTO [SPDK_KanpusEF].[dbo].[DTL_PPB] " +
                        ...
                        " from [SPDK_KanpusEF].[dbo].[DTL_PO] as A" +
                        " inner join [ReferensiEF].[dbo].[KebunBudidaya] as B on A.MerkId=B.KebunBudidayaId" +
                        " where HDR_POId IN (" + strPO + ") and A.SudahFaktur=0",
                    _menuId: menuId
                },
                type: 'POST',
                datatype: 'json'
...
function ModelToView(_NomorInputView) {
    ...
    var lstFaktur = $('#ListFakturId').data('kendoMultiSelect');
    lstFaktur.dataSource.read().done(function (data) {
        lstFaktur.value(_NomorInputView.ListFakturId);
        $('#ListNo_Faktur').val(_NomorInputView.ListNo_Faktur);
        if (_NomorInputView.BuyerId.toString().toLowerCase() == unilever.toLowerCase()) {
            $('#KhususUnilever').css('display', 'block');
        }
        else {
    ...
    var lstPO = $('#ListPOId').data('kendoMultiSelect');
    lstPO.dataSource.read().done(function (data) {
        lstPO.value(_NomorInputView.ListPOId);
...
function NamaBuyerOnSelect(e) {
    ...
    if (buyer.BuyerId.toString().toLowerCase() == unilever.toLowerCase()) {
        $('#ListFakturId').data('kendoMultiSelect').dataSource.read().done(function () {
            $('#ListPOId').data('kendoMultiSelect').dataSource.read().done(function () {
                $('#KhususUnilever').css('display', 'block');
            });
        });
...
function filterPO() {
    ...
    if (headerBaru == true && kebunId != '00000000-0000-0000-0000-000000000000') {
        return {
            strClassView: 'Ptpn8.Penjualan.Models.HDR_PO',
            scriptSQL: "select DISTINCT A.* from [SPDK_KanpusEF].[dbo].[HDR_PO] as A" +
                " inner join [SPDK_KanpusEF].[dbo].[DTL_PO] as B on A.HDR_POId=B.HDR_POId" +
                " inner join [ReferensiEF].[dbo].[KebunBudidaya] as C on B.MerkId=C.KebunBudidayaId" +
                " where A.VerKaur=1 and A.TahunBuku=" + $('#TahunBuku').val() + " and A.BuyerId='" + $("#BuyerId").val() + "' and B.SudahFaktur=0 and C.KebunId='" + $("#KebunId").val() + "'",
            _menuId: menuId
        }
    }
    else if (headerBaru == true && kebunId == '00000000-0000-0000-0000-000000000000') {
        return {
            ...
            scriptSQL: "select DISTINCT A.* from [SPDK_KanpusEF].[dbo].[HDR_PO] as A" +
                " inner join [SPDK_KanpusEF].[dbo].[DTL_PO] as B on A.HDR_POId=B.HDR_POId" +
                " inner join [ReferensiEF].[dbo].[KebunBudidaya] as C on B.MerkId=C.KebunBudidayaId" +
                " where A.VerKaur=1 and A.TahunBuku=" + $('#TahunBuku').val() + " and A.BuyerId='" + $("#BuyerId").val() + "' and B.SudahFaktur=0",
            _menuId: menuId
        }
    }
    else if (headerBaru == false && kebunId != '00000000-0000-0000-0000-000000000000') {
        return {
            ...
            scriptSQL: "select DISTINCT A.* from [SPDK_KanpusEF].[dbo].[HDR_PO] as A" +
                " inner join [SPDK_KanpusEF].[dbo].[DTL_PO] as B on A.HDR_POId=B.HDR_POId" +
                " inner join [ReferensiEF].[dbo].[KebunBudidaya] as C on B.MerkId=C.KebunBudidayaId" +
                " where A.VerKaur=1 and A.TahunBuku=" + $('#TahunBuku').val() + " and A.BuyerId='" + $("#BuyerId").val() + "' and (B.SudahFaktur=0 or A.HDR_POId in (" + listId + ")) and C.KebunId='" + $("#KebunId").val() + "'",
            _menuId: menuId
        }
    }
    else {
        return {
            ...
            scriptSQL: "select DISTINCT A.* from [SPDK_KanpusEF].[dbo].[HDR_PO] as A" +
                " inner join [SPDK_KanpusEF].[dbo].[DTL_PO] as B on A.HDR_POId=B.HDR_POId" +
                " inner join [ReferensiEF].[dbo].[KebunBudidaya] as C on B.MerkId=C.KebunBudidayaId" +
                " where A.VerKaur=1 and A.TahunBuku=" + $('#TahunBuku').val() + " and A.BuyerId='" + $("#BuyerId").val() + "' and (B.SudahFaktur=0 or A.HDR_POId in (" + listId + "))",
            _menuId: menuId
        }
    }
}
