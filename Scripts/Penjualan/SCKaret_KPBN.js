...
var rowNumber = 0;
var menuId = '701d1c83-703a-43e7-1043-1e3bba2e350d';
var dataSource;
...
$(document).ready(function () {
	...
	var h2 = parseInt($(".hdr").css("height")) + parseInt($("._headerkaret").css("height"));
...
function ModelToView(_NomorInputView) {
    ...
    $('#Tanggal_SC').data('kendoDateTimePicker').value(_NomorInputView.Tanggal_SC);
...
function btnDownloadOnClick() {
    downloadInsertGridFrom().done(function (results) {
        if (results.length > 0) {
            ...
            ds.Tanggal_SC = new Date(parseInt(ds.Tanggal_SC.substr(6)));
            $("#grdHDRSC").data("kendoGrid").setDataSource(dataSource);
            $("#grdHDRSC").data("kendoGrid").refresh();
        }
...
function btnPilihOnClick() {
    ...
    _NomorInputView.NamaMainBudidaya = 'KARET';
    ModelToView(_NomorInputView);
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        //contentType: 'application/json; charset=utf-8',
        data: {
            strClassView: "Ptpn8.Penjualan.Models.View_DTLSC",
            scriptSQL:
                "SELECT A.DTL_SCId,A.HDR_SCId,A.SubBudidayaId,B.Nama as NamaSubBudidaya, " +
                    "A.MerkId,C.Merk as NamaMerk,A.QtySacks,A.SatuanId,E.Nama as NamaSatuan, " +
                    "A.GradeId,D.Nama as NamaGrade,A.Netto,A.KK,A.KA,A.ALB,A.Price,A.Price*A.Netto/100 as JumlahHarga, " +
                    "1 as NoBaris FROM [dbo].[DTL_SC] A JOIN [dbo].[SubBudidaya] B ON A.SubBudidayaId=B.SubBudidayaId " +
...
function downloadInsertGridFrom() {
    var grd = $('#grdHDRSC').data('kendoGrid');
    var url = '/Input/GetDataFrom';
    ...
    return $.ajax({
        ...
        //contentType: 'application/json; charset=utf-8',
        data: {
            strClassView: "Ptpn8.Penjualan.Models.View_HDRSC",
            scriptSQL: "select * from [dbo].[HDR_SC] where TahunBuku=" + $('#TahunBuku').val() + " and MainBudidayaId='01EB6E4D-31D2-4ECF-A143-D109A71A3EC8' and No_SC='" + $('#No_SC').val() + "'",
            _menuId: menuId,
            _cs: "csKPBN"
        },
        type: 'POST',
