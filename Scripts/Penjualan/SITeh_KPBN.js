var wndLeaveGrid, wnd, wndDetail, prt, err, del, wndDownload;
var vessel;
...
var gridStatus;
var rowNumber = 0;
var menuId = '6576e946-42f8-387c-1b33-85ce84e6a5e9';
var dataSource;
...
$(document).ready(function () {
    ...
    wndDownload = $("#downloadWindow").kendoWindow({
        title: "Download Data KPBN",
        modal: true,
        visible: false,
        resizable: false,
        width: 800
                            ...
                            PeriksaConstraintHeader().done(function (data) {
                                if (data == 0) {
                                    InsertGridFrom().done(function (results) {
                                        ...
                                        grd.dataSource.data(results);
                                        grd.dataSource.sync();
                                        var currentData = grd.dataSource.data();
...
function ModelToView(_NomorInputView) {
    ...
    $('#Tanggal_SI').data('kendoDateTimePicker').value(_NomorInputView.Tanggal_SI);
    ...
    $('#NotifyAddress6').val(_NomorInputView.NotifyAddress6);
    $('#NotifyAddress7').val(_NomorInputView.NotifyAddress7);
    $('#NotifyAddress8').val(_NomorInputView.NotifyAddress8);
...
function ViewToModel() {
    var mdl = {
        ...
        JenisContainer: $('#JenisContainer').val(),
        ...
        NotifyAddress6: $('#NotifyAddress6').val(),
        NotifyAddress7: $('#NotifyAddress7').val(),
        NotifyAddress8: $('#NotifyAddress8').val(),
        No_PO: $('#No_PO').val(),
...
//function NamaProsesOnSelect(e) {
//    var proses = $('#ProsesStuffing').val();
//}
...
function filterNotifyAddress5() {
    return {
        key: "NotifyAddress5",
        value: $("#NotifyAddress5").val(),
        _menuId: menuId
    };
}
...
function NotifyAddress5OnSelect(e) {
    var consignee = this.dataItem(e.item);
    $('#NotifyAddress6').val(consignee.NotifyAddress6);
    $('#NotifyAddress7').val(consignee.NotifyAddress7);
    $('#NotifyAddress7').val(consignee.NotifyAddress8);
}
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        //contentType: 'application/json; charset=utf-8',
        data: {
            strClassView: "Ptpn8.Penjualan.Models.View_DTLShippingInstruction",
            scriptSQL: "SELECT A.DTL_ShippingInstructionId,A.HDR_ShippingInstructionId,A.DTL_SCId, A.SubBudidayaId, B.Nama as NamaSubBudidaya,A.LineNumber, " +
                "A.MerkId, C.Merk as NamaMerk,A.Chop, A.QtySacks, A.SatuanId, E.Nama as NamaSatuan,A.GradeId, D.Nama as NamaGrade, "+
                "A.Gross, A.Tare, A.Gross as Nett, A.Price, A.No_SC, A.Shipping_Mark, A.SudahInvoice, 1 as NoBaris, A.Price*A.Gross/100 as JumlahHarga, A.KK, A.KA, A.ALB, A.TahunProduksi "+
                "FROM [dbo].[DTL_ShippingInstruction] A JOIN [dbo].[SubBudidaya] B ON A.SubBudidayaId=B.SubBudidayaId " +
                    "JOIN [dbo].[KebunBudidaya] C ON A.MerkId=C.KebunBudidayaId JOIN [dbo].[Grade] D ON A.GradeId=D.GradeId " +
                    "JOIN [dbo].[Satuan] E ON A.SatuanId=E.SatuanId " +
                    "WHERE HDR_ShippingInstructionId='" + $('#HDR_ShippingInstructionId').val() + "'",
            _menuId: menuId,
            _cs: "csKPBN"
        },
...
function btnDownloadOnClick() {
    downloadInsertGridFrom().done(function (results) {
        if (results.length > 0) {
            dataSource = new kendo.data.DataSource({ data: results });
            dataSource.read();
            var ds = dataSource._data[0];
            ds.Tanggal_SI = new Date(parseInt(ds.Tanggal_SI.substr(6)));
            $("#grdHDRSI").data("kendoGrid").setDataSource(dataSource);
            $("#grdHDRSI").data("kendoGrid").refresh();
        }
        wndDownload.open().center();
    });
}

function btnPilihOnClick() {

    _NomorInputView = dataSource._data[0];
    _NomorInputView.NomorInput = $('#NomorInput').val();
    _NomorInputView.NomorInputView = $('#NomorInputView').val();
    _NomorInputView.NamaMainBudidaya = 'TEH';
    ModelToView(_NomorInputView);

    wndDownload.close();
}

function downloadInsertGridFrom() {
    var grd = $('#grdHDRSI').data('kendoGrid');
    var url = '/Input/GetDataFrom';
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "select * from [dbo].[HDR_ShippingInstruction] where TahunBuku=" + $('#TahunBuku').val() + " and MainBudidayaId='3D662350-146F-48B0-A181-5A840330C451' and No_SI='" + $('#No_SI').val() + "'",
            _menuId: menuId,
            _cs: "csKPBN"
        },
        type: 'POST',
