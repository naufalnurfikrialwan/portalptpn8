var wndLeaveGrid, wnd, wndDetail, prt, err, del, wndKirimMemo;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var kirim_MemoDinasId
var menuId = '76633740-79E0-4077-829E-7B45EBD31AC2';
        ...
        $scope.validate = function (event) {
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    kirimMemo(_NomorInputView.Kirim_MemoDinasId, $('#Nomor_MemoDinas').val(),
                        $('#TanggalDikirim_MemoDinas').data('kendoDateTimePicker').value().getDate().toString() + '/' + ($('#TanggalDikirim_MemoDinas').data('kendoDateTimePicker').value().getMonth() + 1).toString() + '/' + $('#TanggalDikirim_MemoDinas').data('kendoDateTimePicker').value().getFullYear().toString(),
                        $('#UserName').val(), _NomorInputView.TahunAgenda_SuratMasuk, _NomorInputView.NomorAgenda_SuratMasuk,
                        $('#TahunBuku').val(), $('#UnitId').val()).done(function (data1) {
                            if (data1 == 'true') {
                            ...
                            window.location.href = "/Input/Input?aplikasiId=7b8b79fa-04e7-40a3-a7b5-d1de6bca2b83&menuId=664cd944-fc12-404c-ae93-6164b538920d";
                            //if (headerBaru) {
...
function tambahNomorBaru() {
    //scriptSQL: "exec [dbo].[UbahStatusMemoDariFinalJadiKirim] '" + id + "'," + nomemo.toString() + ",'" + tglkirim + "', '" + namauser + "'," + thn + "," + nmr + "," + thnbuku + ",'" + divid + "'",
...
function ModelToView(_NomorInputView) {
    ...
    if (typeof (_NomorInputView.StatusMemoDinas) == 'undefined') {
        return;
    }
...
function ViewToModel() {
    ...
    if (typeof (_NomorInputView) == "undefined")
        return null;
    else {
        ...
        var mdl = {
            ...
            CCTujuanId_MemoDinas: _NomorInputView.CCTujuanId_MemoDinas.toString(),
            CCNamaTujuan_MemoDinas: _NomorInputView.CCNamaTujuan_MemoDinas,
...
function readSuratMasuk() {
    ambilSuratMasuk($('#TahunAgenda_SuratMasuk').data('kendoNumericTextBox').value(),
        ...
        if (dta.length > 0) {
            ...
            $('#Isi_MemoDinasDraft').data('kendoEditor').value("-");
            $('#Isi_MemoDinasBaru').data('kendoEditor').value("-");
...
//function kirimMemo(id, tglkirim, namauser, thn, nmr) {
//    var url = '/Input/ExecSQL';
//    return $.ajax({
//        url: url,
//        data: {
//            scriptSQL: "exec [dbo].[UbahStatusMemoDariFinalJadiKirim] '" + id + "', '" + tglkirim + "', '" + namauser + "'," + thn + "," + nmr,
//            _menuId: menuId
//        },
//        type: 'POST',
//        datatype: 'json'
...
function kirimMemo(id, nomemo, tglkirim, namauser, thn, nmr, thnbuku, divid) {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [SPDK_KanpusEF]..[UbahStatusMemoDariFinalJadiKirim] '" + id + "'," + nomemo.toString() + ",'" + tglkirim + "', '" + namauser + "'," + thn + "," + nmr + "," + thnbuku + ",'" + divid + "'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
