...
var menuId = '7CA09FA5-8D9A-4651-9E1D-8FC84087495A';
...
angular.module("__header", ["kendo.directives"])
    .controller("MyCtrl", function($scope) {
        $scope.validate = function(event) {
            ...
            var mdl = ViewToModel();
            mdl.StatusMemoDinas = 'KIRIM';
            if ($scope.validator.validate()) {
                simpanHeader(mdl).done(function(data) {
                    //if (headerBaru) {
                    //    kirimMemo(mdl.Kirim_MemoDinasId, mdl.Tanggal_MemoDinas.substring(0, 10),
                    //        $('#UserName').val(), mdl.TahunBuku, '0', $('#UnitId').val()).done(function() {
                    //            InitHeader();
                    //        });
...
function simpanHeader(mdl) {
    ...
    return $.ajax({
        ...
        data: {
            ...
            _baru: headerBaru,
            _menuId: menuId,
            _SQLLanjut: "exec [dbo].[UbahStatusMemoDariFinalJadiKirimDireksi] '" + mdl.Kirim_MemoDinasId + "', '" +
                        mdl.Tanggal_MemoDinas.substring(0, 10) + "', '" + $('#UserName').val() + "'," + mdl.TahunBuku + "," + '0' + ",'" + $('#UnitId').val() + "'"
        },
        type: 'POST',
        datatype: 'json'
    });
...
function kirimMemo(id, tglkirim, namauser, thn, nmr, unitid) {
    var url = '/Input/ExecSQL';
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [dbo].[UbahStatusMemoDariFinalJadiKirimDireksi] '" + id + "', '" + tglkirim + "', '" + namauser + "'," + thn + "," + nmr + ",'" + unitid+"'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function filterTujuan(e) {
    ...
    //var _scriptSQL = "SELECT * FROM [ReferensiEF].[dbo].[Unit] where "+
    //    "(IndukOrgId in ('3FC61718-BA8D-4A2B-B851-B07E10D4520A', '78A299AF-9733-412B-835D-083953E67E0D')  " +
    //    "OR IndukOrgId in (SELECT[DireksiId] FROM [ReferensiEF].[dbo].[Direksi]) OR IndukOrgId IN (SELECT WILAYAHID FROM [ReferensiEF].[dbo].[Wilayah])) and NamaUnit not like '%Bidang%' "+
    //    "and NamaUnit like '%" + $("#NamaTujuan_MemoDinas").val() + "%'";
...
function ModelToView(_NomorInputView) {
    ...
    stringDecode(_NomorInputView.Isi_MemoDinasFinal).done(function (dta) {
        _NomorInputView.Isi_MemoDinasFinal = dta;
            $('#Isi_MemoDinasBaru').data('kendoEditor').value(_NomorInputView.Isi_MemoDinasFinal);
            $('#Isi_MemoDinasDraft').data('kendoEditor').value(_NomorInputView.Isi_MemoDinasFinal);
            $('#Isi_MemoDinasFinal').data('kendoEditor').value(_NomorInputView.Isi_MemoDinasFinal);
            $('#__editor1').css("display", "none");
            $('#__editor2').css("display", "none");
...
function ViewToModel() {
    ...
    $('#Lampiran_MemoDinas').val(listLampiranMemoDinas.toString())
    $('#Isi_MemoDinasBaru').val($('#Isi_MemoDinasFinal').val());
    $('#Isi_MemoDinasDraft').val($('#Isi_MemoDinasFinal').val());
