...
var hdr_ProyekId, dtl_ProyekId;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var rowNumber = 0;
var menuId = '6bdf3693-40b1-4f52-82d6-a9d6bd4c0c11';
...
function ModelToView(_NomorInputView) {
    $('#HDR_ProyekId').val(_NomorInputView.HDR_ProyekId);
    ...
    $('#TanggalRegister').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.TanggalRegister.substr(6))));
    $('#NamaProyek').val(_NomorInputView.NamaProyek);
    $('#Lokasi').val(_NomorInputView.Lokasi);
    $('#Luas').val(_NomorInputView.Luas);
    $('#PerkiraanNilai').val(_NomorInputView.PerkiraanNilai);
...
function ViewToModel() {
    var mdl = {
        ...
        TanggalRegister: $('#TanggalRegister').val(),
        NamaProyek: $('#NamaProyek').val(),
        Lokasi: $('#Lokasi').val(),
        Luas: $('#Luas').val(),
        PerkiraanNilai: $('#PerkiraanNilai').val(),
...
function PeriksaConstraintHeader() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from [KerjasamaEF].[dbo].[Kerjasama_DTLProyek]" +
                " where HDR_ProyekId='" + $("#HDR_ProyekId").val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        if (typeof (e.item) != 'undefined')
            ...
            hdr_ProyekId = _NomorInputView.HDR_ProyekId;
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_ProyekId: $('#HDR_ProyekId').val() }
    });
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_ProyekId = model.uid;
        model.HDR_ProyekId = hdr_ProyekId;
    }
    dtl_ProyekId = model.DTL_ProyekId;
    gridStatus = 'sudah-diapa-apain';
}
...
function ddlPersyaratanKerjasamaOnChange(e) {
    var persyaratan = $('#Persyaratan').val();
    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(dtl_ProyekId)) == "undefined" ? grid.dataSource.getByUid(dtl_ProyekId) : grid.dataSource.get(dtl_ProyekId);
