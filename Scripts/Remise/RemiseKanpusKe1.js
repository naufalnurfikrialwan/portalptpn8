...
var rowNumber = 0;
var menuId = '98d9c299-4e5f-4707-bec5-aef16c83bea6';
var IHTBukan = 'Bukan';
                    ...
                    if (data) {
                        ...
                        getDataFrom3().done(function (results) {
                            if (results == 0) {
                                ...
                                alert('Data Belum Diverifikasi oleh Kebun/Unit');
...
function ModelToView(_NomorInputView) {
    ...
    $('#PeriodeRemise').val('AWAL');
    $('#StatusFinal').data('kendoDropDownList').value(_NomorInputView.StatusFinal);
}
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "select A.*, B.Nama as NamaKebun, '' as NomorInputView from [SPDK_KanpusEF].[dbo].[HDR_Remise] A " +
                ...
                "where BulanRemise=" + $('#BulanRemise').val() + " and TahunRemise=" + $('#TahunRemise').val() + " and A.KebunId='" + $('#KebunId').val() + "' and PeriodeRemise='AWAL' and NomorInput=" + $('#NomorInput').val(),
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function getDataFrom2() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "exec RemisePencapaianProduksi '" + $('#KebunId').val() + "', " + $('#BulanRemise').val() + ", " + $('#TahunRemise').val(),
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function getDataFrom3() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from  [SPDK_KanpusEF].[dbo].[DTL_RemiseKebutuhanModalKerja] A JOIN [SPDK_KanpusEF].[dbo].[HDR_Remise] B ON A.HDR_RemiseId=B.HDR_RemiseId" +
                " where A.HDR_RemiseId='" + $("#HDR_RemiseId").val() + "' and B.VerAdsi=1",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "EXEC remisehitunggaji '" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + "; EXEC PROC_RemiseKebutuhanModalKerja '" + $('#HDR_RemiseId').val() +
                    "','" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'" + IHTBukan + "', 'DIKANPUS'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function grdOnBound(e) {
    ...
    for (var j = 0; j < dataItems.length; j++) {
        ...
        warnaKolomDivisi(dataItems[j],"Tekpol");
        warnaKolomDivisi(dataItems[j],"Tanaman");
        warnaKolomDivisi(dataItems[j],"HukumUmum");
        warnaKolomDivisi(dataItems[j],"Tekpol");
        warnaKolomDivisi(dataItems[j],"SDM");
        warnaKolomDivisi(dataItems[j],"Sekper");
        warnaKolomDivisi(dataItems[j],"Akuntansi");
        warnaKolomDivisi(dataItems[j],"Wilayah");

    }
}

function warnaKolomDivisi(dataItems,divisi) {

    var grd = $('#grdDetail').data('kendoGrid');
    if (dataItems.get("StatusDisetujui_"+divisi) == "BELUM") {
        columnIndex = grd.wrapper.find(".k-grid-header [data-field=" + "Disetujui_"+divisi + "]").index();
        row = grd.tbody.find("[data-uid='" + dataItems.uid + "']");
        cell = row.children().eq(columnIndex);
        cell.addClass("rowStyleBelumDibuka");

        columnIndex = grd.wrapper.find(".k-grid-header [data-field=" + "Disetujui_"+divisi+"Pct" + "]").index();
        row = grd.tbody.find("[data-uid='" + dataItems.uid + "']");
        cell = row.children().eq(columnIndex);
        cell.addClass("rowStyleBelumDibuka");
    } else if (dataItems.get("StatusDisetujui_"+divisi) == "SUDAH" && dataItems.get("Disetujui_"+divisi) == dataItems.get("UsulanKebun")) {
        columnIndex = grd.wrapper.find(".k-grid-header [data-field=" + "Disetujui_"+divisi + "]").index();
        row = grd.tbody.find("[data-uid='" + dataItems.uid + "']");
        cell = row.children().eq(columnIndex);
        cell.addClass("rowStyleSudahDibukaTidakEdit");
        ...
        row = grd.tbody.find("[data-uid='" + dataItems.uid + "']");
        cell = row.children().eq(columnIndex);
        cell.addClass("rowStyleSudahDibukaEdit");
