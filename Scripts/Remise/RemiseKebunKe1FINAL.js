...
var rowNumber = 0;
var menuId = 'BA3F9991-98AA-4B2D-AE2D-AE2C44707B77';
var IHTBukan = 'Bukan';
        ...
        $scope.validate = function (event) {
            ...
            if ($scope.validator.validate()) {
                //simpanHeader().done(function (data) {
                    ...
                    //else {
                    //    openErrWindow();
                    //    $('#tabstrib').addClass('disabledbutton');
                    //    gridDestroy();
                    //}
                //})
                //.fail(function (x) {
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        getDataFrom4().done(function (results) {
            ...
            if (_NomorInputView.StatusFinal == "SUDAH FINAL") {
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL:  "EXEC remisehitunggaji '" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + "; "+
                        "EXEC PROC_RemiseKebutuhanModalKerja '" + $('#HDR_RemiseId').val() + "','" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'" + IHTBukan +"','AWAL','DIKEBUN'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
