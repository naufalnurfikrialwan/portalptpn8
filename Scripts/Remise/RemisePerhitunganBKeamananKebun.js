...
var rowNumber = 0;
var menuId = '01f4cba3-0e2b-6a38-3efc-5b1d3f7048e5';
var IHTBukan = 'Bukan';
    ...
    $('#grdDetail').kendoValidator({
        rules: {
            validated: function (input) {
                if (input.attr("name") == "JumlahBiaya") {
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    if (data) {
                        ...
                        grid.dataSource.read({id: $('#HDR_RemiseBiayaKeamananId').val()});
                    }
                    else {
                        openErrWindow();
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_RemiseBiayaKeamananId: $('#HDR_RemiseBiayaKeamananId').val()}
    });
