...
var hdr_VerifikasiUploadDataHarianId, kebunId, dtl_VerfikasiUploadDataHarianId;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var rowNumber = 0;
var menuId = '2D0F74C8-7E06-4EB5-B0E8-0393F4A5D357';
        ...
        $scope.validate = function (event) {
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    if (data) {
                        hdr_VerifikasiUploadDataHarianId = $('#HDR_VerifikasiUploadDataHarianId').val();
                        $('#NomorInputView').data('kendoDropDownList').dataSource.read().done(function () {
                            ...
                            $("#grdDetail").data("kendoGrid").dataSource.read({
                                id: $('#HDR_VerifikasiUploadDataHarianId').val()
                            });
                        });
                    }
                    else {
...
function ModelToView(_NomorInputView) {
    $('#HDR_VerifikasiUploadDataHarianId').val(_NomorInputView.HDR_VerifikasiUploadDataHarianId);
    $('#KebunId').val(_NomorInputView.KebunId);
    $('#NamaKebun').val(_NomorInputView.NamaKebun);
    $('#Tanggal_Transaksi').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.Tanggal_Transaksi.substr(6))));
    ...
    $('#No_Verifikasi').val(_NomorInputView.No_Verifikasi);    
}
...
function ViewToModel() {
    var mdl = {
        ...
        NomorInputView: Array(5 - String($('#NomorInput').val()).length + 1).join('0') + $('#NomorInput').val() + ' - ' + $('#No_Verifikasi').val().toUpperCase(),
        No_Verifikasi: $('#No_Verifikasi').val().toUpperCase()       
    };
    return mdl;
}
...
function PeriksaConstraintHeader() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from [SPDK_KanpusEF].[dbo].[DTL_VerifikasiUploadDataHarianId]" +
                " where HDR_VerifikasiUploadDataHarianId='" + $("#HDR_VerifikasiUploadDataHarianId").val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        hdr_VerifikasiUploadDataHarianId = _NomorInputView.HDR_VerifikasiUploadDataHarianId;
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_PermintaanBarangDariKebunId: $('#HDR_VerifikasiUploadDataHarianId').val() }
    });
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_VerifikasiUploadDataHarianId = model.uid;
        model.HDR_VerifikasiUploadDataHarianId = hdr_VerifikasiUploadDataHarianId;
    }
    dtl_VerifikasiUploadDataHarianId = model.DTL_VerifikasiUploadDataHarianId;
    kebunId = $('#KebunId').val();
    gridStatus = 'sudah-diapa-apain';
}
...
function filterBudidaya(e) {
    ...
    function aucNamaBudidayaOnSelect(e) {
        var ddlItem = this.dataItem(e.item);
        var grid = $('#grdDetail').data('kendoGrid');
        var dataItem = typeof (grid.dataSource.get(dtl_VerifikasiUploadDataHarianId)) == "undefined" ? grid.dataSource.getByUid(dtl_VerifikasiUploadDataHarianId) : grid.dataSource.get(dtl_VerifikasiUploadDataHarianId);
        var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
        var data = grid.dataItem(row);
        data.MainBudidayaId = ddlItem.MainBudidayaId;
        mainBudidayaId = ddlItem.MainBudidayaId;
    }

    function aucNamaBudidayaOnDataBound(e) {
