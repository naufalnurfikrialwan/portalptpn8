var reservasiId, reservasi_TambahanFasilitasId;
var wndLeaveGrid, wnd, wndDetail, prt, err, del;
var model;
var headerBaru, detailBaru;
var gridStatus;
var mainBudidayaId;
var menuId = '43cafd59-70a6-46bc-8dc4-a28c79b43a80';
...
function grdDetailReservasiOnEdit(e) {
    ...
    if (model.isNew()) {
        model.id = model.uid;
        e.container.find("input[name=Reservasi_TambahanFasilitasId]").val(model.id).change(); // trigger change in order to notify the model binding
        e.container.find("input[name=ReservasiId]").val(reservasiId).change(); // trigger change in order to notify the model binding
    }
    uid = model.uid;
    reservasi_TambahanFasilitasId = model.id;
}
...
function sendDataDTL() {
    ...
    for (var i = 0; i < currentData.length; i++) {
        if (currentData[i].isNew()) {
            ...
            updatedRecords.push(JSON.stringify(currentData[i]));
            spupdated.push("EXEC Agrowisata_UbahHargaTagihanPelunasan '" + kodeBooking + "'");
        }
    }
    ...
    $.extend(data,
        ...
        parameterMap({ classdv: 'Ptpn8.Agrowisata.Models.View_ReservasiTambahanFasilitas' })
        );
...
function aucReservasiTambahanFasilitasOnSelect(e) {
    var ddlItem = this.dataItem(e.item);
    var grid = $('#grdDetailReservasi_' + reservasiId).data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(reservasi_TambahanFasilitasId)) == "undefined" ? grid.dataSource.getByUid(reservasi_TambahanFasilitasId) : grid.dataSource.get(reservasi_TambahanFasilitasId);
    var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
    var data = grid.dataItem(row);
    data.ReservasiId = reservasiId;
    data.Reservasi_TambahanFasilitasId = reservasi_TambahanFasilitasId;
    data.Nama = ddlItem.Nama;
    data.Agrowisata_FasilitasTambahanId = ddlItem.Agrowisata_FasilitasTambahanId;
}

function aucReservasiTambahanFasilitasOnDataBound(e) {

}

function filterReservasiTambahanFasilitas(e) {
    return {
        strClassView: 'Ptpn8.Agrowisata.Models.View_ReservasiTambahanFasilitas',
        scriptSQL: "select distinct Nama, Agrowisata_FasilitasTambahanId from [SPDK_KanpusEF].[dbo].[Agrowisata_FasilitasTambahan] order by Nama asc",
        _menuId: menuId
    }
}