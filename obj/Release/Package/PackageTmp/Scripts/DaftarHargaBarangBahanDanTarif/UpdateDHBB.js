...
var rowNumber = 0;
var menuId = '469A28BE-E012-46B5-B4D3-38880D85AF37';
                ...
                if (input.attr("name") == "HargaDasar" && input.val() != "") {
                    cekHargaDasar(dtl_UpdateDHBBId).done(function (data) {
                        if (data.length > 0) {
                            ...
                            var tanggalsurvey = dataItem.TanggalSurvey;
                            var hargaDasar = data[0][0];
                            var objek_tgl = new Date();

                            var Nomonths;
                            Nomonths = (objek_tgl.getFullYear() - tanggalsurvey.getFullYear()) * 12;
                            Nomonths -= tanggalsurvey.getMonth() + 1;
...
function cekHargaDasar(dtl_UpdateDHBBId) {
    var url = '/Input/ExecSQL';
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select HargaDasar from [SPDK_KanpusEF].[dbo].[DTL_UpdateDHBB]" +
                " where dtl_UpdateDHBBId='" + dtl_UpdateDHBBId + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function ModelToView(_NomorInputView) {
    ...
    $('#KelompokBarang').data('kendoDropDownList').value(_NomorInputView.KelompokBarang);
}
...
function KelompokBarangOnSelect(e) {
    var bulanSurvey = $('#KelompokBarang ').val();
}
