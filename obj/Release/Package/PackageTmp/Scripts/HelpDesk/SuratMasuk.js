...
var hdr_SuratMasukId, dtl_SuratMasukId;
var _NomorInputView;
var model;
        ...
        $scope.validate = function (event) {
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    if (data) {
                        hdr_SuratMasukId = $('#HDR_SuratMasukId').val();
...
function ModelToView(_NomorInputView) {
    $('#HDR_SuratMasukId').val(_NomorInputView.HDR_SuratMasukId);
    ...
    $('#No_Bulan').val(_NomorInputView.No_Bulan);
...
function ViewToModel() {
    var mdl = {
        ...
        NomorInputView: Array(5 - String($('#NomorInput').val()).length + 1).join('0') + $('#NomorInput').val() + ' - ' + $('#No_Bulan').val().toUpperCase(),
        No_Bulan: $('#No_Bulan').val().toUpperCase(),
...
function PeriksaConstraintHeader() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from [SPDK_KanpusEF].[dbo].[DTL_SuratMasuk]" +
                " where HDR_SuratMasukId='" + $("#HDR_SuratMasukId").val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        if (typeof (e.item) != 'undefined')
            ...
            hdr_SuratMasukId = _NomorInputView.HDR_SuratMasukId;
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_SuratMasukId = model.uid;
        model.HDR_SuratMasukId = hdr_SuratMasukId;
    }
    dtl_SuratMasukId = model.DTL_SuratMasukId;
    gridStatus = 'sudah-diapa-apain';
}
...
function ddlBagianOnChange(e) {
    ...
    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(dtl_SuratMasukId)) == "undefined" ? grid.dataSource.getByUid(dtl_SuratMasukId) : grid.dataSource.get(dtl_SuratMasukId);
    var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
    var data = grid.dataItem(row);
}

function ddlPetugasOnChange(e) {
    var ditujukanStaf = $('#Ditujukan_Staf').val();
    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(dtl_SuratMasukId)) == "undefined" ? grid.dataSource.getByUid(dtl_SuratMasukId) : grid.dataSource.get(dtl_SuratMasukId);
    var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
    var data = grid.dataItem(row);
...
function ddlKasubagOnChange(e) {
    var ditujukanKasubag = $('#Ditujukan_Kasubag').val();
    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(dtl_SuratMasukId)) == "undefined" ? grid.dataSource.getByUid(dtl_SuratMasukId) : grid.dataSource.get(dtl_SuratMasukId);
    var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
    var data = grid.dataItem(row);
...
function ddlJenisSuratOnChange(e) {
...
function ddlDisposisiOnChange(e) {
