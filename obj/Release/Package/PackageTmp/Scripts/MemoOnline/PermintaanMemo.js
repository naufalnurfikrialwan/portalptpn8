...
var buyer;
var hdr_MemoOnlineIdb;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var rowNumber = 0;
var menuId = '655B298E-E1EA-4BB5-B349-76233E000C62';
    ...
    $("#grdDetail").kendoValidator({
        rules: { // custom rules
            chopvalidation: function (input) {
                if (input.attr("name") == "Norek" && input.val() != "") {
                    cekNorek(input.val()).done(function (data) {
                        if (data.length > 0) {
                            ...
                            dataItem.NamaRekening = data[0][2];
                            $('#NamaRekening').val(data[0][2]);
                        }
                    });
                }else if(input.attr("name") == "NamaBarang" && input.val() != "") {
...
function filterNamaBarang(e) {
    ...
    var rekeningId = data.RekeningId;
    return {
