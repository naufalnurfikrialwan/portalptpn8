    ...
    var rowNumber = 0;
	var menuId='CA742DAC-05D3-4029-B250-47FF4D7EA159';
        ...
        $("#grdDetail").kendoValidator({
            rules: { // custom rules
                chopvalidation: function (input) {
					if(input.attr("name") == "NamaSubBudidaya" && input.val() != "") {
...
$(window).on("unload", function () {
    ...
    {
        if (typeof (_NomorInputView.Tanggal_PO) != 'undefined') {
...
function cariUserYangLagiNgedit()
{
    ...
    function ddlBudidayaOnSelect(e) {
        ...
        var mainBudidayaId = $('#MainBudidayaId').val();
        $('#BuyerId').data('kendoAutoComplete').dataSource.read();
    }
