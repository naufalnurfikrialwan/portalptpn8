var alokasiPengemudiId, err, berangkat, berangkatdasar, month, year, date;
...
var gridStatus;
var jumlahHari, jenisTujuan, kmdanmenginap, dapatPenginapan;
var jenisTujuan, jarak, asalId, tujuanId, tujuan, jenistujuan2;
                ...
                if (input.attr("name") == "NIK" || input.attr("name") == "NamaDriver" || input.attr("name") == "Golongan" || input.attr("name") == "Jabatan") {
                    cekNIK($('#NIK').val()).done(function (data) {
                        if (data.length > 0) {
                            ...
                            //var dapatPenginapan = $('#DapatPenginapan').val();
                            var kendaraanDinas = 1;
                            ...
                            if (golongan1 == "IA" || golongan1 == "IB" || golongan1 == "IC" || golongan1 == "ID") {
                                if (jabatan.toLowerCase().indexOf("karyawan") > -1 || jabatan.toLowerCase().indexOf("kary") > -1 || jabatan.toLowerCase().indexOf("jtu") > -1 || jabatan.toLowerCase().indexOf("mandor") > -1 || jabatan.toLowerCase().indexOf("pengemudi") > -1) { jabatan1 = "Gol_I" }
                            }
                            ...
                            if (jenisTujuan == 'Dalam Wilayah') {
                                ...
                                if (kmdanmenginap == "Menginap < 100 km" || kmdanmenginap == "Menginap 100-200 km" || kmdanmenginap == "Menginap > 100 km") {
                                    ...
                                    cSQL = "select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi1 + ") group by uraianbiaya" +
                                        ...
                                        " union select uraianbiaya,sum(" + jabatan1 + ")*0 * " + jumlahHari + " from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi4 + ")  group by uraianbiaya order by uraianbiaya";
                                        ...
                                        cSQL = "select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi1 + ") group by uraianbiaya" +
                                            " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi2 + ") group by uraianbiaya" +
                                            " union select uraianbiaya,sum(" + jabatan1 + ")*0 * " + jumlahHari + " from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi3 + ")  group by uraianbiaya order by uraianbiaya";
                                ...
                                if (kendaraanDinas == 1) {
                                    ...
                                    " union select uraianbiaya,sum(" + jabatan1 + ")*0 * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi5 + ")  group by uraianbiaya order by uraianbiaya";
                                }
                            }
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.AlokasiPengemudiId = model.uid;
    }
    alokasiPengemudiId = model.AlokasiPengemudiId;
    ...
    if (jenisTujuan == "Jakarta" && jumlahHari == 0) { jenisTujuan = "Jakarta PP" }
    else if (jenisTujuan == "Jakarta" && jumlahHari > 0) { jenisTujuan = "Jakarta Nginap" }

    if (jarak < 51 && jumlahHari == 0) { kmdanmenginap = "Tdk Menginap < 50 km" }
    else if (jarak > 50 && jarak < 100 && jumlahHari == 0) { kmdanmenginap = "Tdk Menginap < 100 km" }
    else if (jarak > 99 && jarak < 200 && jumlahHari == 0) { kmdanmenginap = "Tdk Menginap 100-200 km" }
    else if (jarak > 199 && jumlahHari == 0) { kmdanmenginap = "Tdk Menginap > 200 km" }
    if (jarak < 51 && jumlahHari > 0) { kmdanmenginap = "Menginap < 50 km" }
    else if (jarak > 50 && jarak < 100 && jumlahHari > 0) { kmdanmenginap = "Menginap < 100 km" }
    else if (jarak > 99 && jarak < 200 && jumlahHari > 0) { kmdanmenginap = "Menginap 100-200 km" }
    else if (jarak > 199 && jumlahHari > 0) { kmdanmenginap = "Menginap > 200 km" }
    else if (jarak == null && jumlahHari > 0 && jenisTujuan == "Dalam Wilayah") { kmdanmenginap = "Menginap < 100 km" }
    else if (jarak == null && jumlahHari == 0 && jenisTujuan == "Dalam Wilayah") { kmdanmenginap = "Tdk Menginap < 100 km" }
    ...
    else if ((jenisTujuan == "Jakarta PP" || jenisTujuan == "Jakarta Nginap") && (jumlahHari == 0 || jumlahHari > 0)) { kmdanmenginap = "Jakarta" }

    var grid = $('#grdDetail').data('kendoGrid');
    var dataItem = typeof (grid.dataSource.get(alokasiPengemudiId)) == "undefined" ? grid.dataSource.getByUid(alokasiPengemudiId) : grid.dataSource.get(alokasiPengemudiId);
    dataItem.KMdanMenginap = kmdanmenginap;
    ...
    if (jenisTujuan == "Dalam Wilayah") {
        if (tujuandlmwilayah == "Lain-Lain < 100" || tujuandlmwilayah == "Lain-Lain > 100" || tujuandlmwilayah == "Lain-Lain 100-200") {
    ...
    else if (jenisTujuan == "Luar Wilayah < 100 km" || "Luar Wilayah > 100 km") {
        $(e.container).find("input[name='Tujuan']").prop('disabled', false);
    ...
    else if (jenisTujuan == "Jakarta Nginap" || "Jakarta PP") {
        $(e.container).find("input[name='Tujuan']").prop('disabled', true);
        $(e.container).find("input[name='TujuanDalamWilayah']").prop('disabled', true);
        //e.container.find("input[name='TujuanDalamWilayah']").data("kendoDropDownList").enable(false);
    }
    ...
    return $.ajax({
        ...
        data: {
            scriptSQL: "select top 1 Jarak from [ReferensiEF].[dbo].[Jarak]" +
                " where AsalId='977EF1BF-D05F-4680-9080-58093FBE7D95' and TujuanId ='" + tujuanId + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function btnPrintHeaderOnClick(e) {
    ...
    var data = grid.dataItem(row);
    var datalokasipengemudiid = data.AlokasiPengemudiId;

    var viewer = $("#reportViewer").data("telerik_ReportViewer");
    viewer.reportSource({
        report: viewer.reportSource().report,
        parameters: { AlokasiPengemudiId: datalokasipengemudiid }
    });
...
function aucLokasiKerjaTujuanOnSelect(e) {
    ...
    var row = grid.tbody.find("tr[data-uid='" + dataItem.uid + "']");
    var data = grid.dataItem(row);
    data.AsalId = '977EF1BF-D05F-4680-9080-58093FBE7D95';
    data.AsalDalamWilayah = 'Divisi Hukum dan Umum';
