...
var menuId = 'F01F9770-4981-412B-A7D5-B96EA7B808CA';
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    ...
                    if (data) {
                        ...
                        window.location.href = "/Input/Input?aplikasiId=7b8b79fa-04e7-40a3-a7b5-d1de6bca2b83&menuId=0dedeaed-9a05-4fa0-9b21-b9c61b3e5f62";
                    }
                    else {
                        openErrWindow();
                    }
...
function simpanHeader() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            _baru: headerBaru,
            _menuId: menuId,
            _SQLLanjut: "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariDraftJadiSiapFinal] '" + $('#Kirim_MemoDinasId').val() + "', " + $('#TahunBuku').val() +
            ", '" + $('#UnitId').val() + "', '" + $('#NamaTujuan_MemoDinas').val() + "', '" + $('#Perihal_MemoDinas').val() + "'; " +
            "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariDistribusiJadiBalas] '" + kirim_MemoDinasIdBALAS + "', '" + $('#UserName').val() + "'"
        },
        type: 'POST',
        datatype: 'json'
    });
...
function cekData(nomorInputView) {
    ...
    {
        _NomorInputView.StatusMemoDinas = 'DRAFT';
    ...
    {
        headerBaru = false;
        nomorInputView.StatusMemoDinas = 'DRAFT';
        $('#StatusMemoDinas').val('DRAFT');
        disableHeader();
...
function ModelToView(_NomorInputView) {
    ...
    if (_NomorInputView.StatusMemoDinas == "BARU" || _NomorInputView.StatusMemoDinas == "") _NomorInputView.StatusMemoDinas = "DRAFT";
...
function ViewToModel() {
    ...
    $('#Lampiran_MemoDinas').val(listLampiranMemoDinas.toString());
    $('#Pemeriksa_MemoDinas').val($('#NamaUser').val());
    $('#TanggalDiperiksa_MemoDinas').val('@DateTime.Now');
...
function InitHeader() {
    ...
    //$('#Balasan_MemoDinasId').val('');
    //$('#NomorBalasan_MemoDinas').val('');
    //$('#PenandaTangan_MemoDinas').val('');
    //$('#TanggalDitandatangan_MemoDinas').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.TanggalDitandatangan_MemoDinas.substr(6))));
    //$('#Pembuat_MemoDinas').val('');
    //$('#TanggalDibuat_MemoDinas').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.TanggalDibuat_MemoDinas.substr(6))));
    //$('#Pemeriksa_MemoDinas').val('');
    //$('#TanggalDiperiksa_MemoDinas').data('kendoDateTimePicker').value(new Date(parseInt(_NomorInputView.TanggalDiperiksa_MemoDinas.substr(6))));
...
function disableHeader() {
    ...
    if (typeof ($('#Isi_MemoDinasDraft').data('kendoEditor')) != 'undefined')
        $($('#Isi_MemoDinasDraft').data().kendoEditor.body).attr('contenteditable', false);
}

function enableHeader() {
    if ($('#StatusMemoDinas').val() == 'DRAFT' || $('#StatusMemoDinas').val() == 'BARU') {
        ...
        $('#btnDeleteHeader').removeClass('disabledbutton');
        $('#btnPrintHeader').removeClass('disabledbutton');
        $($('#Isi_MemoDinasDraft').data().kendoEditor.body).attr('contenteditable', true);
    }
    else {
        disableHeader();
    }
...
function readSuratMasuk() {
    ambilSuratMasuk($('#TahunAgenda_SuratMasuk').data('kendoNumericTextBox').value(),
        ...
        if (dta.length > 0) {
            ...
            var editor;
            editor = $('#Isi_MemoDinasDraft').data('kendoEditor');
...
function ubahStatus() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariDraftJadiSiapFinal] '" + $('#Kirim_MemoDinasId').val() + "', " + $('#TahunBuku').val() +
                ", '" + $('#UnitId').val() + "', '" + $('#NamaTujuan_MemoDinas').val() + "', '" + $('#Perihal_MemoDinas').val() + "'",
            _menuId: menuId
        },
        type: 'POST',
