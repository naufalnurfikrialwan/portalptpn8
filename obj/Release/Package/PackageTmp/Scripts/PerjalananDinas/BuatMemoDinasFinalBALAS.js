...
var kirim_MemoDinasIdBALAS;
var menuId = '2522B8BB-FBB5-44DC-B97B-1D9E8900273F';
...
function ambilContext() {
    var url = '/Input/bacaContextParameter';
    return $.ajax({
        url: url,
        data: {
            namaContext: 'ParameterUntukBalasMemo' + $('#UserName').val()
        },
        type: 'POST',
        datatype: 'json'
    });
...
$(document).ready(function () {
    ...
    InitHeader();
    disableHeader();
    ambilContext().done(function (data) {
        kirim_MemoDinasIdBALAS = data;
    });
            ...
            if ($scope.validator.validate()) {
                simpanHeader().done(function (data) {
                    ...
                    if (data) {
                        ...
                        window.location.href = "/Input/Input?aplikasiId=7b8b79fa-04e7-40a3-a7b5-d1de6bca2b83&menuId=0dedeaed-9a05-4fa0-9b21-b9c61b3e5f61";
                    }
                    else {
                        openErrWindow();
                    }
...
function tambahNomorBaru() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [SPDK_KanpusEF].[dbo].[UbahTambahNomorMemo] " + $('#TahunBuku').val() + ", '" + $('#UnitId').val() + "'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function simpanHeader() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            _baru: headerBaru,
            _menuId: menuId,
            _SQLLanjut: "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariFinalJadiSiapKirim] '" + $('#Kirim_MemoDinasId').val() + "', " + $('#TahunBuku').val() +
            ", '" + $('#UnitId').val() + "', '" + $('#NamaTujuan_MemoDinas').val() + "', '" + $('#Perihal_MemoDinas').val() + "'; " +
            "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariTerimaJadiBalas] '" + kirim_MemoDinasIdBALAS + "', '" + $('#UserName').val()+"'"
        },
        type: 'POST',
        datatype: 'json'
    });
...
function NomorInputViewOnSelect(e) {
    ...
    ubahStatusNotif($('#UserName').val(), $('#Kirim_MemoDinasId').val()).done(function () {
        enableHeader();
...
function cekData(nomorInputView) {
    ...
    {
        _NomorInputView.StatusMemoDinas = 'FINAL';
        _NomorInputView.PenandaTangan_MemoDinas = _NomorInputView.Pembuat_MemoDinas;  
        _NomorInputView.Pemeriksa_MemoDinas = _NomorInputView.Pembuat_MemoDinas;  
    ...
    {
        headerBaru = false;
        nomorInputView.StatusMemoDinas = 'FINAL';
        $('#StatusMemoDinas').val('FINAL');
        disableHeader();
    ...
    if (!adaDataBaru) {
        cariUserYangLagiNgedit().done(function (data) {
            if (data != "")
                ...
                $('#msgInputView').text('Tidak dapat membuat data baru');
        }).fail(function () {
...
function ModelToView(_NomorInputView) {
    ...
    if (_NomorInputView.StatusMemoDinas == 'BARU') {
        stringDecode(_NomorInputView.Isi_MemoDinasBaru).done(function (data) {
            $('#Isi_MemoDinasBaru').data('kendoEditor').value(data);
        ...
        stringDecode(_NomorInputView.Isi_MemoDinasDraft).done(function (data) {
            $('#Isi_MemoDinasDraft').data('kendoEditor').value(data);
        ...
        stringDecode(_NomorInputView.Isi_MemoDinasFinal).done(function (data) {
            $('#Isi_MemoDinasFinal').data('kendoEditor').value(data);
            $('#Isi_MemoDinasBaru').val(_NomorInputView.Isi_MemoDinasBaru);
            $('#Isi_MemoDinasDraft').val(_NomorInputView.Isi_MemoDinasDraft);
            $('#__editor1').css("display", "none");
            $('#__editor2').css("display", "none");
    ...
    if (_NomorInputView.StatusMemoDinas == "DRAFT" || _NomorInputView.StatusMemoDinas == "") _NomorInputView.StatusMemoDinas = "FINAL";
    ...
    $('#SifatSurat_MemoDinas').data('kendoDropDownList').value(_NomorInputView.SifatSurat_MemoDinas);
...
function ViewToModel() {
    ...
    $('#Lampiran_MemoDinas').val(listLampiranMemoDinas.toString())
    $('#PenandaTangan_MemoDinas').val($('#NamaUser').val());
    //$('#TanggalDitandatangan_MemoDinas').val('@DateTime.Now');
...
function disableHeader() {
    ...
    if (typeof ($('#Isi_MemoDinasFinal').data('kendoEditor')) != 'undefined')
        $($('#Isi_MemoDinasFinal').data().kendoEditor.body).attr('contenteditable', false);
}

function enableHeader() {
    if ($('#StatusMemoDinas').val() == 'DRAFT' || $('#StatusMemoDinas').val() == 'FINAL') {
        ...
        $('#btnDeleteHeader').removeClass('disabledbutton');
        $('#btnPrintHeader').removeClass('disabledbutton');
        $($('#Isi_MemoDinasFinal').data().kendoEditor.body).attr('contenteditable', true);
    }
    else {
        disableHeader();
    }
...
function readSuratMasuk() {
    ambilSuratMasuk($('#TahunAgenda_SuratMasuk').data('kendoNumericTextBox').value(),
        ...
        if (dta.length > 0) {
            ...
            var editor;
            editor = $('#Isi_MemoDinasFinal').data('kendoEditor');
...
function ubahStatus() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [SPDK_KanpusEF].[dbo].[UbahStatusMemoDariFinalJadiSiapKirim] '" + $('#Kirim_MemoDinasId').val() + "', " + $('#TahunBuku').val() +
                ", '" + $('#UnitId').val() + "', '" + $('#NamaTujuan_MemoDinas').val() + "', '" + $('#Perihal_MemoDinas').val() + "'",
            _menuId: menuId
        },
        type: 'POST',
