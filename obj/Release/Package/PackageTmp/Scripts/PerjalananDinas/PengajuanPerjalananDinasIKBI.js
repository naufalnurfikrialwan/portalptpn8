...
var hdr_PermintaanPerjalananDinasIKBIId, kebunId, dtl_PermintaanPerjalananDinasIKBIId, reg, berangkat;
var _NomorInputView;
var model;
var headerBaru, detailBaru;
var rowNumber = 0;
var menuId = '36c558f7-c158-4219-b5c9-897e062ff4c7';
var tahunPermintaan, bulanPermintaan;
    ...
    $("#grdDetail").kendoValidator({
        rules: { // custom rules
            validation: function (input) {
                if (input.attr("name") == "NIK" || input.attr("name") == "Golongan" || input.attr("name") == "Jabatan") {
                    cekNIK($('#NIK').val()).done(function (data) {
                        if (data.length > 0) {
                            var grid = $('#grdDetail').data('kendoGrid');
                            var dataItem = typeof (grid.dataSource.get(dtl_PermintaanPerjalananDinasIKBIId)) == "undefined" ? grid.dataSource.getByUid(dtl_PermintaanPerjalananDinasIKBIId) : grid.dataSource.get(dtl_PermintaanPerjalananDinasIKBIId);
                            if (data[0][1] == 01)
                            { golongan = "IA" }
                            else if (data[0][1] == 02)
                            { golongan = "IB" }
                            ...
                            var jumlahHari = (((((pisahTanggalAkhir - pisahTanggalAwal) / 1000) / 60) / 60) / 24);
                            ...
                            if (golongan1 == "IA" || golongan1 == "IB" || golongan1 == "IC" || golongan1 == "ID") {
                                ...
                                if (jabatan.toLowerCase().indexOf("staf") > -1 || jabatan.toLowerCase().indexOf("asisten afdeling") > -1 || jabatan.toLowerCase().indexOf("asisten pengolahan") > -1 || jabatan.toLowerCase().indexOf("asisten administrasi") > -1 || jabatan.toLowerCase().indexOf("asisten tata usaha") > -1 || jabatan.toLowerCase().indexOf("asisten teknik") > -1) { jabatan1 = "Gol_III" }
                            }
                            ...
                            if (jenisTujuan == 'Dalam Wilayah') {
                                if (dapatBiayaPenginapan == 0) {
                                    if (kmdanmenginap == "Menginap < 50 km") {
                                        ...
                                        cSQL = "select uraianbiaya,sum(" + jabatan1 + ")* 0 * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi1 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahMakan + ") from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi2 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ")* 0 * " + jumlahHari + " from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi3 + ")  group by uraianbiaya order by uraianbiaya";
                                    }
                                    ...
                                    if (kmdanmenginap == "Menginap < 50 km") {
                                        ...
                                        cSQL = "select uraianbiaya,sum(" + jabatan1 + ") * 0 * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi1 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahMakan + ") from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi2 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * 0 * " + jumlahHari + " from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi3 + ")  group by uraianbiaya" +
                                ...
                                if (kendaraanDinas == 0) {
                                    if (kmdanmenginap == "Menginap < 100 km" || kmdanmenginap == "Menginap 100-200 km" || kmdanmenginap == "Menginap > 200 km") {
                                        if (dapatBiayaPenginapan == 0) {
                                            ...
                                            cSQL = "select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) * 0 from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi1 + ") group by uraianbiaya" +
                                            " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahMakan + ") from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi2 + ") group by uraianbiaya" +
                                            ...
                                            " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + ") from [ReferensiEF].[dbo].[BPD] where StatusWilayah='Luar Wilayah' and (" + kondisi5 + ")  group by uraianbiaya order by uraianbiaya";
                                        }
                                    }
                                    else {
                                        kondisi1 = "uraianbiaya='Uang Saku'"
                                ...
                                if (kendaraanDinas == 1) {
                                    ...
                                    if (dapatBiayaPenginapan == 0) {
                                        ...
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi2 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + ") from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi3 + ") group by uraianbiaya" +
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * (" + jumlahHari + "+1) from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi4 + ")  group by uraianbiaya order by uraianbiaya";
                                    }
                                    else if (dapatBiayaPenginapan == 1) {
                                        kondisi1 = "uraianbiaya='Uang Saku'";
                                        kondisi2 = "uraianbiaya='Uang Makan'";
                                        ...
                                        " union select uraianbiaya,sum(" + jabatan1 + ") * " + jumlahHari + " from [ReferensiEF].[dbo].[BPD] where StatusWilayah='" + jenisTujuan + "' and (" + kondisi5 + ")  group by uraianbiaya order by uraianbiaya";
                                    }
                                }
                            }
                                ...
                                if (jenisDinas == 1) {
                                    if (jenisTujuan == 'Luar Wilayah < 100 km' || jenisTujuan == 'Luar Wilayah > 100 km') {
                                        if (data.length == 5) {
                                            ...
                                            dataItem["UangMakan"] = parseFloat(data[3][1]) * 0.8;
                                            dataItem["UangSaku"] = parseFloat(data[4][1]) * 0.8;
                                            dataItem["JumlahBPD"] = (parseFloat(data[3][1]) + parseFloat(data[4][1])) * 0.8;
                                        }
                                        else if (data.length == 4) {
                                            dataItem.dirty = true;
                                            dataItem["UangCucian"] = 0;
                                            dataItem["UangPenginapan"] = 0;
                                            dataItem["UangMakan"] = parseFloat(data[2][1]) * 0.8;
                                            dataItem["UangSaku"] = parseFloat(data[3][1]) * 0.8;
                                            dataItem["JumlahBPD"] = (parseFloat(data[2][1]) + parseFloat(data[3][1])) * 0.8;
                                            dataItem["UangTransport"] = 0;
                                        }
                                            ...
                                            if (kendaraanDinas == 1) {
                                                dataItem.dirty = true;
                                                dataItem["UangTransport"] = 0;
                                                dataItem["UangMakan"] = parseFloat(data[1][1]) * 0.8;
                                                dataItem["UangSaku"] = parseFloat(data[2][1]) * 0.8;
                                                dataItem["JumlahBPD"] = (parseFloat(data[1][1]) + parseFloat(data[2][1])) * 0.8;
                                                dataItem["UangCucian"] = 0;
                                                dataItem["UangPenginapan"] = 0;
                                            }
                                            ...
                                            dataItem.dirty = true;
                                            dataItem["UangMakan"] = parseFloat(data[0][1]) * 0.8;
                                            dataItem["UangSaku"] = parseFloat(data[1][1]) * 0.8;
                                            dataItem["UangCucian"] = 0;
                                            dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]))* 0.8;
                                            dataItem["UangTransport"] = 0;
                                            dataItem["UangPenginapan"] = 0;
                                        }
                                    }
                                        ...
                                        if (data.length == 4) {
                                            ...
                                            dataItem["UangSaku"] = 0;
                                            dataItem["UangCucian"] = 0;
                                            dataItem["JumlahBPD"] = (parseFloat(data[0][1])) * 0.8;
                                            dataItem["UangTransport"] = 0;
                                            dataItem["UangPenginapan"] = 0;
                                        }
                                        else {
                                    ...
                                    //----------------------------------------------- ---------------------------------------------------------------------------
                                else if (jenisDinas == 0) {
                                    if (jenisTujuan == 'Luar Wilayah < 100 km' || jenisTujuan == 'Luar Wilayah > 100 km') {
                                        if (data.length == 5) {
                                            dataItem.dirty = true;
                                            dataItem["UangCucian"] = parseFloat(data[0][1]) * 0.8;
                                            dataItem["UangPenginapan"] = parseFloat(data[1][1]) * 0.8;
                                            dataItem["UangTransport"] = parseFloat(data[2][1]) * 0.8;
                                            dataItem["UangMakan"] = parseFloat(data[3][1]) * 0.8;
                                            dataItem["UangSaku"] = parseFloat(data[4][1]) * 0.8;
                                            dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]) + parseFloat(data[2][1]) + parseFloat(data[3][1]) + parseFloat(data[4][1])) * 0.8;
                                        }
                                        else if (data.length == 4) {
                                            dataItem.dirty = true;
                                            dataItem["UangCucian"] = parseFloat(data[0][1]) * 0.8;
                                            ...
                                            dataItem["UangMakan"] = parseFloat(data[2][1]) * 0.8;
                                            dataItem["UangSaku"] = parseFloat(data[3][1]) * 0.8;
                                            dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]) + parseFloat(data[2][1]) + parseFloat(data[3][1])) * 0.8;
                                            dataItem["UangTransport"] = 0;
                                        }
                                        else if (data.length == 3) {
                                             if (kendaraanDinas == 1) {
                                                 ...
                                                 dataItem["UangMakan"] = parseFloat(data[1][1]) * 0.8;
                                                 dataItem["UangSaku"] = parseFloat(data[2][1]) * 0.8;
                                                 dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]) + parseFloat(data[2][1])) * 0.8;
                                                 dataItem["UangTransport"] = 0;
                                                 dataItem["UangPenginapan"] = 0;
                                            }
                                            else if (kendaraanDinas == 0) {
                                                dataItem.dirty = true;
                                                dataItem["UangTransport"] = parseFloat(data[0][1]) * 0.8;
                                                dataItem["UangMakan"] = parseFloat(data[1][1]) * 0.8;
                                                dataItem["UangSaku"] = parseFloat(data[2][1]) * 0.8;
                                                dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]) + parseFloat(data[2][1])) * 0.8;
                                                dataItem["UangCucian"] = 0;
                                        ...
                                        if (data.length == 5) {
                                            ...
                                            if (kendaraanDinas == 1) {
                                                ...
                                                dataItem.dirty = true;
                                                dataItem["UangCucian"] = parseFloat(data[0][1]) * 0.8;
                                                dataItem["UangTransport"] = parseFloat(data[1][1]) * 0.8;
                                                dataItem["UangMakan"] = parseFloat(data[2][1]) * 0.8;
                                                dataItem["UangSaku"] = parseFloat(data[3][1]) * 0.8;
                                                dataItem["JumlahBPD"] = (parseFloat(data[0][1]) + parseFloat(data[1][1]) + parseFloat(data[2][1]) + parseFloat(data[3][1])) * 0.8;
                                                dataItem["UangPenginapan"] = 0;
    ...
    return $.ajax({
        ...
        data: {
            scriptSQL: "select top 1 A.Nama, A.Gol, A.MK, A.Nama_Jab from [SPDK_KanpusEF].[dbo].[Ref_Dik] as A " +
                ...
                " where B.KebunId='" + kebunId + "' and A.reg='" + dataItem.NIK + "' and Year(A.Tanggal)=" + tahunPermintaan + " and Month(A.Tanggal)=" + bulanPermintaan + "" +
        ...
        $scope.validate = function (event) {
            ...
            if ($scope.validator.validate()) {
                ...
                simpanHeader().done(function (data) {
                    if (data) {
                        hdr_PermintaanPerjalananDinasIKBIId = $('#HDR_PermintaanPerjalananDinasIKBIId').val();
                        ...
                        $('#NomorInputView').data('kendoDropDownList').dataSource.read().done(function () {
                            ...
                            $("#grdDetail").data("kendoGrid").dataSource.read({
                                id: $('#HDR_PermintaanPerjalananDinasIKBIId').val()
                            });
                            getDataFrom();
                        });
                    }
...
function ModelToView(_NomorInputView) {
    $('#HDR_PermintaanPerjalananDinasIKBIId').val(_NomorInputView.HDR_PermintaanPerjalananDinasIKBIId);
    ...
    $('#SuratPenugasan').val(_NomorInputView.SuratPenugasan);
    ...
    $('#JenisDinas').data('kendoDropDownList').value(_NomorInputView.JenisDinas);
...
function ViewToModel() {
    ...
    else if (jarak < 100 && jumlahHari2 > 0)
    ...
    var mdl = {
        ...
        SuratPenugasan: $('#SuratPenugasan').val(),
        ...
        JenisDinas: $('#JenisDinas').data('kendoDropDownList').value(),
        Jarak: jarak
    };
    return mdl;
}
...
function PeriksaConstraintHeader() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from [SPDK_KanpusEF].[dbo].[DTL_PermintaanPerjalananDinasIKBI]" +
                " where HDR_PermintaanPerjalananDinasIKBIId='" + $("#HDR_PermintaanPerjalananDinasIKBIId").val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        hdr_PermintaanPerjalananDinasIKBIId = _NomorInputView.HDR_PermintaanPerjalananDinasIKBIId;
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_PermintaanPerjalananDinasIKBIId: $('#HDR_PermintaanPerjalananDinasIKBIId').val() }
    });
...
function filterNomorInput(e) {
    ...
    return { _model: mdl, _menuId: '36c558f7-c158-4219-b5c9-897e062ff4c7' };
}
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_PermintaanPerjalananDinasIKBIId = model.uid;
        model.HDR_PermintaanPerjalananDinasIKBIId = hdr_PermintaanPerjalananDinasIKBIId;
    }
    dtl_PermintaanPerjalananDinasIKBIId = model.DTL_PermintaanPerjalananDinasIKBIId;
    kebunId = $('#KebunId').val();
...
function SimpanData(status, dta) {
    ...
    var url = '/Input/ExecSQL';
    var DTL_PermintaanPerjalananDinasIKBIId = dta.DTL_PermintaanPerjalananDinasIKBIId;

    var HDR_PermintaanPerjalananDinasIKBIId = hdr_PermintaanPerjalananDinasIKBIId;
    var NIK = dta.NIK;
    var Nama = dta.Nama;
    ...
    if (status == 'baru') {
        return $.ajax({
            url: url,
            data: {
                scriptSQL: "exec dbo.PerjalananDinas_PengajuanIKBI '" + DTL_PermintaanPerjalananDinasIKBIId.toString() +
                    "','" + HDR_PermintaanPerjalananDinasIKBIId.toString() + "','" + NIK + "','" + Nama + "','" + Golongan + "','" + Jabatan + "','" +
        ...
        return $.ajax({
            url: url,
            data: {
                scriptSQL: "exec dbo.PerjalananDinas_PengajuanIKBI_Edit '" + DTL_PermintaanPerjalananDinasIKBIId.toString() +
                    "','" + HDR_PermintaanPerjalananDinasIKBIId.toString() + "','" + NIK + "','" + Nama + "','" + Golongan + "','" + Jabatan + "','" +
                    UangSaku + "', '" + UangMakan + "', '" + UangCucian + "', '" + UangTransport + "', '" + UangLain + "', '" + JumlahBPD + "', '" + UangPenginapan + "'",
                _menuId: menuId
            },
        ...
        return $.ajax({
            url: url,
            data: {
                scriptSQL: "exec dbo.PerjalananDinas_PengajuanIKBI_Del '" + DTL_PermintaanPerjalananDinasIKBIId.toString() + "'",
                _menuId: menuId
            },
            type: 'POST',
            datatype: 'json'
...
function LokasiTujuan(e) {
    ...
    $('#TujuanDalamWilayah').data('kendoDropDownList').value(namatujuan);
    ...
    return $.ajax({
        ...
        data: {
            scriptSQL: "select count(WilayahId) as Data from [ReferensiEF].[dbo].[Kebun]" +
                " where WilayahId='" + $("#AsalId").val() + "' and KebunId ='" + $("#TujuanId").val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function JenisDinasOnSelect(e) {
    var jenisDinas = $('#JenisDinas').val();
}

function filterPengemudi() {
    var tglPermintaan = new Date(parseInt(_NomorInputView.TanggalPermintaan.substr(6)));
    var bulanpermintaan = tglPermintaan.getMonth() + 1;
    var tahunpermintaan = tglPermintaan.getFullYear();
    return {
        strClassView: 'Ptpn8.UploadDataHarian.Models.ref_dik',
        scriptSQL: "select distinct NAMA, REG from [SPDK_KanpusEF].[dbo].[Ref_Dik]" +
            " where (NAMA_JAB like '%pengemudi%' or NAMA_JAB like '%sopir%' or NAMA_JAB like '%supir%') and KD_KBN = '" + $('#Kd_Kbn').val() + "' and MBT= 'K' and Month(Tanggal) = '" + bulanpermintaan + "' and Year(Tanggal) = '" + tahunpermintaan + "'" +
            " union select distinct NAMA, REG from [SPDK_KanpusEF].[dbo].[Ref_DikKLM]" +
            " where (NAMA_JAB like '%pengemudi%' or NAMA_JAB like '%sopir%' or NAMA_JAB like '%supir%') and KD_KBN = '" + $('#Kd_Kbn').val() + "' and Month(Tanggal) = '" + bulanpermintaan + "' and Year(Tanggal) = " + tahunpermintaan,
        _menuId: menuId
    }
}
