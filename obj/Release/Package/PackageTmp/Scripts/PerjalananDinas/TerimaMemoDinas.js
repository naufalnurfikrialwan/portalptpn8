...
var userName;
var menuId = '0dedeaed-9a05-4fa0-9b21-b9c61b3e5f60';
var dataROW;
...
$(function () {
    ...
    hubMemoDinas.client.terimaMemoDinas = function (data) {
...
$(document).ready(function () {
    ...
    wndTerimaMemo = $("#wTerimaMemo").kendoWindow({
        title: "Terima Memo",
        modal: true,
        visible: false,
        resizable: false,
...
function terimaMemo(id,unitid,noterima,tglterima,namauser) {
    var url = '/Input/ExecSQL';
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "exec [SPDK_KanpusEF]..[UbahNotifJadiTerbaca] '" + namauser + "','" + menuId + "','" + id + "'; exec [SPDK_KanpusEF]..[UbahNotifJadiTerbaca] '" + namauser + "','" + menuId + "','" + id + "'; exec [SPDK_KanpusEF]..[UbahStatusMemoDariKirimJadiTerima] '" + id + "', '" + unitid + "'," + noterima + ", '" + tglterima + "', '" + namauser + "'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function openWindowTerimaMemo(e) {
    e.preventDefault();
    ...
    execAmbilNomorBaru().done(function (dta) {
        wndTerimaMemo.open().center();
        $('#_NomorTerima_MemoDinas').val(dta);
        $('#_NomorTerima_MemoDinas').data('kendoNumericTextBox').focus();
        $('#_TanggalTerima_MemoDinas').data('kendoDateTimePicker').value(__data.TanggalTerima_MemoDinas);
...
function execAmbilNomorBaru() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select top 1 NomorBaru_Terima from [SPDK_KanpusEF].[dbo].[eOffice_Nomor_MemoDinas] where TahunBuku='" + $('#TahunBuku').val() + "' and UnitId='" + $('#UnitId').val() + "'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function btnTerimaMemoDinasOnClick(e) {
    e.preventDefault();
    ...
    var tgl = $('#_TanggalTerima_MemoDinas').val();
    terimaMemo(__data.Kirim_MemoDinasId, __data.UnitId, $('#_NomorTerima_MemoDinas').val(),
        tgl.substr(0,2) + '/' + tgl.substr(3,2) + '/' + tgl.substr(6,4),
        $('#namaUser').val()).done(function (dta) {
            if (dta == 'true') {
                $('#statusTerima').text('Sukses');
                hubMemoDinas.server.terimaMemoDinas('7B8B79FA-04E7-40A3-A7B5-D1DE6BCA2B83');
                wndTerimaMemo.close();
                grid.dataSource.read();
            }
            else {
                $('#statusTerima').text('Gagal Terima!!!!');
            }
        });
}

function btnBatalTerimaMemoDinasOnClick(e) {
    wndTerimaMemo.close();
}
