...
var rowNumber = 0;
var menuId = '51770018-6FE4-4A40-90A5-116816102EC1';
var IHTBukan = 'Bukan';
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "EXEC PROC_ArusKasPKB '" + $('#HDR_PKBId').val() + "','" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'AWAL'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function hitungSUM(namaField) {
    ...
    for (var i = 0; i < sortedData.length; i++) {
        var strEdit = sortedData[i]["EditDisetujui"];
...
function grdOnBound(e) {
    ...
    for (var j = 0; j < dataItems.length; j++) {
        ...
        if (dataItems[j].get("EditPengajuan") == "readonly" || dataItems[j].get("EditPengajuan").indexOf("sum") > -1) {
            columnIndex = grd.wrapper.find(".k-grid-header [data-field=" + "Nilai_Kandir" + "]").index();
            row = grd.tbody.find("[data-uid='" + dataItems[j].uid + "']");
            cell = row.children().eq(columnIndex);
            cell.addClass("rowStyle");
