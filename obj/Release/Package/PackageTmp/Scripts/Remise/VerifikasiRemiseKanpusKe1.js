...
var rowNumber = 0;
var menuId = '5a119c49-268f-4804-83d5-1b654bc16556';
var IHTBukan = 'Bukan';
                        ...
                        getDataFrom3().done(function (results) {
                            if (results == 0) {
                                InsertGridFrom().done(function (data) {
                                    if (data[0].Keterangan == '1') {
                                        $("#grdDetail").data("kendoGrid").dataSource.read({ id: $('#HDR_RemiseId').val() });
                                    }
                                    else {
                                        $("#grdDetail").data("kendoGrid").dataSource.read([]);
                                        alert('Data Belum Di Verifikasi Oleh Kebun/Unit !');
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_VerifikasiBagianRemiseKebutuhanModalKerjaId = model.uid;
        model.BagianId = $('#BagianId').val();
        model.HDR_RemiseId = hdr_RemiseId;
    }

    dtl_RemiseId = model.DTL_VerifikasiBagianRemiseKebutuhanModalKerjaId;
    kebunId = $('#KebunId').val();
    gridStatus = 'sudah-diapa-apain';
...
function getDataFrom2() {
    ...
    return $.ajax({
        ...
        data: {
            ...
            scriptSQL: "exec [SPDK_KanpusEF].[dbo].[RemisePencapaianProduksi] '" + $('#KebunId').val() + "', " + $('#BulanRemise').val() + ", " + $('#TahunRemise').val(),
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function getDataFrom3() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from  [SPDK_KanpusEF].[dbo].[DTL_VerifikasiBagianRemiseKebutuhanModalKerja]" +
                " where HDR_RemiseId='" + $("#HDR_RemiseId").val() + "' and BagianId='"+$('#BagianId').val()+"'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function InsertGridFrom() {
    ...
    return $.ajax({
        ...
        //contentType: 'application/json; charset=utf-8',
        data: {
            strClassView: "Ptpn8.Remise.Models.View_DTLVerifikasiBagianRemiseKebutuhanModalKerja",
            scriptSQL: "EXEC remisehitunggaji '" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + "; " +
                       "EXEC PROC_VerifikasiRemiseKebutuhanModalKerja '" + $('#HDR_RemiseId').val() +"','" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'" + IHTBukan + "','AWAL', 'DIKANPUS','"+bagianId+"'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
