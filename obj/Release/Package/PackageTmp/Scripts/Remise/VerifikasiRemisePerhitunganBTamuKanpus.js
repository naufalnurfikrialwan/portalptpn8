...
var rowNumber = 0;
var menuId = '3C77711F-23D1-4FF1-8268-CD3416BB5702';
var IHTBukan = 'Bukan';
    ...
    $('#grdDetail').kendoValidator({
        rules: {
            validated: function (input) {
                if (input.attr("name") == "JumlahOrang_Disetujui" || input.attr("name") == "JumlahMakanPagi_Disetujui" ||
                    input.attr("name") == "JumlahMakanSiang_Disetujui" || input.attr("name") == "JumlahMakanMalam_Disetujui" || input.attr("name") == "JumlahSnack_Disetujui") {
                    ...
                    var jmlOrang = parseFloat(dataItem["JumlahOrang_Disetujui"]);
                    //var volume = parseFloat(dataItem["Volume_Disetujui"]);
                    //var dptMakanPagi = dataItem["DapatMakanPagi_Disetujui"];
                    //var dptMakanSiang = dataItem["DapatMakanSiang_Disetujui"];
                    //var dptMakanMalam = dataItem["DapatMakanMalam_Disetujui"];
                    //var dptSnack = dataItem["DapatSnack_Disetujui"];
                    var jmlMakanPagi = dataItem["JumlahMakanPagi_Disetujui"];
                    var jmlMakanSiang = dataItem["JumlahMakanSiang_Disetujui"];
                    var jmlMakanMalam = dataItem["JumlahMakanMalam_Disetujui"];
                    var jmlSnack = dataItem["JumlahSnack_Disetujui"];

                    dataItem.dirty = true;

                    dataItem["JumlahBiaya_Disetujui"] = (jmlOrang * jmlMakanPagi * taripMakan) +
                        ...
                        getDataFrom3().done(function (results) {
                            if (results == 0) {
                                InsertGridFrom().done(function (data) {
                                    $("#grdDetail").data("kendoGrid").dataSource.read({ id: $('#HDR_RemiseBiayaTamuId').val() });
                                }).fail(function (x) {
                                    alert('Error! Hubungi Bagian TI');
                                });
                            }
...
function getDataFrom3() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "select count(*) as JmlRecord from  [SPDK_KanpusEF].[dbo].[DTL_VerifikasiBagianRemiseBiayaTamu]" +
                " where HDR_RemiseBiayaTamuId='" + $("#HDR_RemiseBiayaTamuId").val() + "' and BagianId='" + $('#BagianId').val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function InsertGridFrom() {
    ...
    return $.ajax({
        url: url,
        data: {
            strClassView: "Ptpn8.Remise.Models.View_DTLVerifikasiBagianRemiseBiayaTamu",
            scriptSQL: "EXEC PROC_VerifikasiBagianRemiseBiayaTamu '" + $('#HDR_RemiseBiayaTamuId').val() + "','" + $('#KebunId').val() + "'," + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'" + bagianId + "'",
            _menuId: menuId
        },
        type: 'POST',
        datatype: 'json'
...
function sisipKeRemiseTotal() {
    ...
    return $.ajax({
        url: url,
        data: {
            scriptSQL: "EXEC SisipKeRemiseTengahBulanByTamu '" + $('#BagianId').val() + "', " + jmlBiayaDisetujui.toString() + ", " + $('#BulanRemise').val() + "," + $('#TahunRemise').val() + ",'" + $('#KebunId').val() + "'",
            _menuId: menuId
        },
        type: 'GET',
        datatype: 'json'
...
function NomorInputViewOnSelect(e) {
    if (gridStatus == 'sudah-nyoba-disimpan-tapi-model-masih-salah' ||
        ...
        getDataFrom4().done(function (results) {
            ...
            {
                $('#HDR_RemiseBiayaTamuId').val('00000000-0000-0000-0000-000000000000');
...
function btnPrintHeaderOnClick() {
    ...
    viewer.reportSource({
        ...
        parameters: { HDR_RemiseBiayaTamuId: $('#HDR_RemiseBiayaTamuId').val(), KebunId: $('#KebunId').val(), Bulan: $('#BulanRemise').val(), Tahun: $('#TahunRemise').val() }
    });
...
function grdOnEdit(e) {
    ...
    if (model.isNew()) {
        model.DTL_VerifikasiBagianRemiseBiayaTamuId = model.uid;
        model.HDR_RemiseBiayaTamuId = hdr_RemiseId;
    }
    $(e.container).find("input[name='JumlahBPD']").prop('disabled', true);

    dtl_RemiseId = model.DTL_VerifikasiBagianRemiseBiayaTamuId;
    kebunId = $('#KebunId').val();
    gridStatus = 'sudah-diapa-apain';
